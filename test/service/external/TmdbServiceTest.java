...
public class TmdbServiceTest {
  ...
  // testeamos buscar en TMDb por de tvdbId y que encuentre
  @Test
  public void testTmdbServiceFindByTvdbIdOk() {
    ...
    try {
      when(tmdbService.findByTvdbId(81189)).thenReturn(tvShow);
      TvShow tvShow2 = tmdbService.findByTvdbId(81189);

      assertNotNull(tvShow2);
      assertEquals(tvShow.name, tvShow2.name);
    } catch (Exception ex) {
      Logger.warn("No se puede ejecutar el test porque TMDb no responde");
    }
  }

  // testeamos buscar en TMDb por id y que no encuentre
  @Test
  public void testTmdbServiceFindByTvdbNotOk() {
    TvShow tvShow = new TvShow();
    ...
    TmdbService tmdbService = mock(TmdbService.class);
    try {
      when(tmdbService.findByTvdbId(0)).thenReturn(null);
      TvShow tvShow2 = tmdbService.findByTvdbId(0);
